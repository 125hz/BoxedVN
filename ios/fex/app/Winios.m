// Ported verbatim from the pinned integration (app/Mythic/Winios/Winios.m).
//
// This is the application half of the iOS display driver. win32u's
// driver_ios.c declares every winios_* entry point below as a weak extern
// and calls it through a null check, so on the integration's own link the
// absence of this file is survivable. On Darwin a plain weak declaration is
// still a hard reference at static-link time - weak_import is what makes one
// optional - so linking win32u without this produced thirteen undefined
// symbols rather than a driver that politely does nothing.
//
// Taken unmodified so it stays diffable against the integration. Everything
// it needs from outside is already linked: winios_drv_post_mouse,
// winios_drv_post_key and winios_dump_window_tree come from driver_ios.c in
// libwin32u_unix.a, and ios_dump_all_thread_stacks from signal_arm64_ios.c
// in libntdll_unix.a.
