...
struct GuestJoystick: UIViewRepresentable {
    let active: Bool
    @AppStorage("BoxedVN.joystick.arrowKeys") private var arrowKeys = false
...
final class GuestJoystickControl: UIControl {
    ...
    var arrowKeys = false {
        didSet {
            if oldValue != arrowKeys { releaseKeys() }
            accessibilityLabel = arrowKeys ? "Arrow key joystick" : "WASD joystick"
        }
    }
    ...
    override func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
        ...
        var next = Set<String>()
        if distance > 6 {
            if dx > max(6, abs(dy) * 0.45) { next.insert(arrowKeys ? "Right" : "D") }
            if dx < -max(6, abs(dy) * 0.45) { next.insert(arrowKeys ? "Left" : "A") }
            if dy > max(6, abs(dx) * 0.45) { next.insert(arrowKeys ? "Down" : "S") }
            if dy < -max(6, abs(dx) * 0.45) { next.insert(arrowKeys ? "Up" : "W") }
        }
