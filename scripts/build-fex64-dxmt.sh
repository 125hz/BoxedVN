...
#
# Usage:
#   scripts/build-fex64-dxmt.sh [--third-party-dir DIR] [--jobs N] [--force]
#
# DXMT ships in two halves. The PE half (d3d11.dll, dxgi.dll, winemetal.dll)
# is a Windows DLL the guest loads through Wine; we take those prebuilt from
# the pinned integration. This script builds the other half - the Metal and
# shader-translation code that has to run as native iphoneos arm64 - into one
# static archive the application links.
#
# Without it, winemetal.dll's unix calls resolve through the one-entry null
# table in BVNWineRuntimeStubs.c, and the first call the guest does not
# error-check branches to address zero. That is how the x64 graphics target
# died before this existed: d3d11.dll+0x1CE020 jumped to 0x0 one call after
# "Failed to set Metal cache path".
#
...
COMMON_FLAGS=(-arch arm64 -isysroot "${SDK}"
              "-miphoneos-version-min=${IPHONEOS_MIN}" -fblocks -O2)
