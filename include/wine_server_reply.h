 ...
 * and the server replies with startup state. Wine 9 includes the admitted PE
 * entry point; Wine 11 removes that field and moves suspend to offset 8.
 *
...
// REQ_init_process_done in Wine 9.0 and 11.0 generated request lists. The opcode is
...
enum class WineServerProtocol { Wine9, Wine11 };

struct WineInitProcessDoneReply {
    bool valid = false;
    bool entryProvided = true;
...
// Wine 9.0 and 11.0 server_protocol.h: 12-byte request_header, handle at 12,
...
inline WineInitProcessDoneReply decodeWineInitProcessDoneReply(
    const std::uint8_t* message, std::uint64_t length,
    WineServerProtocol protocol = WineServerProtocol::Wine9) noexcept {
    WineInitProcessDoneReply reply;
    reply.entryProvided = protocol == WineServerProtocol::Wine9;
    if (message == nullptr || length < (reply.entryProvided ? 20u : 12u)) {
        return reply;
    }
    ...
    // Wine 11 removed entry; suspend follows the reply header at offset 8.
    if (reply.entryProvided)
        reply.entry = (std::uint64_t)dword(8) | ((std::uint64_t)dword(12) << 32);
    reply.suspend = (std::int32_t)dword(reply.entryProvided ? 16 : 8);
    reply.valid = true;
    return reply;
}

// Wine 9 requires a nonzero entry. Wine 11 no longer sends an entry; admission
// is determined by its status, without interpreting suspend as an address.
inline bool wineInitProcessDoneAdmitted(
    const WineInitProcessDoneReply& reply) noexcept {
    return reply.valid && reply.error == 0 && (!reply.entryProvided || reply.entry != 0);
}
